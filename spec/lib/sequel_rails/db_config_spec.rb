...
describe SequelRails::DbConfig do
  ...
  describe "#url" do
    ...
    it "creates nice URLs" do
      expect(from(adapter: :postgresql,
                  database: :foo, host: 'bar', port: 42,
                  something: 'hi!', user: 'linus'
                 ).url).to\
          eq 'postgres://bar:42/foo?something=hi%21&user=linus'
    end

    it "creates proper JDBC URLs" do
      expect(from(adapter: 'jdbc:postgresql',
                  database: :foo, host: 'bar', port: 42,
                  something: 'hi!', user: 'linus'
                 ).url).to\
          eq 'jdbc:postgresql://bar:42/foo?something=hi%21&user=linus'
    end
